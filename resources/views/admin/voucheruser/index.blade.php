...
@section('content')
    ...
    <div class="card shadow-lg rounded card p-2">
        <div class="card-header" id="#atas">
            ...
            <button type="button" class="btn mb-3 btn-primary float-end" data-bs-toggle="modal" data-bs-target="#vuModal">
                Tambah Stok
            </button>
            @include('admin.voucheruser.create')
        </div>
                    ...
                    <tbody class="table-border-bottom-1">
                        @if (count($voucher_users))
                            @foreach ($voucher_users as $voucher_user)
                                ...
                                <td>
                                    <div class="d-flex">
                                        {{ $voucher_user->payment->name }}
                                    </div>
                                </td>
                                <td>
                                    <div class="d-flex">
                                        {{ $voucher_user->created_at->format('Y-m-d') }}
                                    </div>
                                </td>
                                <td>
                                    <div class="d-flex">
                                        {{ $voucher_user->created_at->format('h:i:s A') }}
                                    </div>
                                </td>

                                <td>
                                    <form action="{{ route('voucheruser.destroy', $voucher_user->id) }}" method="post">
                                        @csrf
                                        @method('delete')
                                        <a href="{{ route('voucheruser.edit', $voucher_user->id) }}"
                                            class="btn btn-sm btn-secondary" data-bs-toggle="tooltip" data-bs-offset="0,4"
